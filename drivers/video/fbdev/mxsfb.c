...
//#analofgics1
//3.2_inch display code starts........................................................................../////////////////////
//#analogics1 for display sk-at 3.2" Display in ptc Landscape diaplay board
...
#define get(X)   gpio_get_value(X) 

#define SCK	 GPIO_TO_PIN(1, 12) //MX6UL_PAD_ENET2_TX_DATA1__GPIO2_IO12
#define CS0	 GPIO_TO_PIN(1, 15)//MX6UL_PAD_ENET2_RX_ER__GPIO2_IO15
#define SDA      GPIO_TO_PIN(1, 13)//MX6UL_PAD_ENET2_TX_EN__GPIO2_IO13
#define RESET    GPIO_TO_PIN(2, 4) //MX6UL_PAD_LCD_RESET__GPIO3_IO04
 
//#define RESET_H   Set(DISP_RESET, 1);
//////#define RESET_L   Set(DISP_RESET, 0);
void LCD_Init(void)
{
 int ret=4;
//	printk("analogics1: func is %s line is %d\n", __func__, __LINE__);
     ret=Gpio_set_value(SCK,"CLOCK",1,0);
	printk("SCK=%d\n",SCK);
     ret=Gpio_set_value(CS0,"CHIPSELECT",1,0);
	printk("CS0=%d",CS0);
     ret=Gpio_set_value(SDA,"DATAIN",1,0);
	printk("SDA=%d\n", SDA);
     ret=Gpio_set_value(RESET,"RESET",1,0);
	printk("RESET=%d\n", RESET);
}
...
{
#if 0
	LCD_DATA(0x28);//Display off
//	mdelay(10);
	LCD_DATA(0x10);//LCD in sleep
	mdelay(20);
	//RGB_OFF();// RGB in standby
#endif
 printk("ENTER_STAND_BY LCD>>>>\n");
}
#if 0
void LCD_Exit_Standby(void)
{
	...
	mdelay(20);
#if 0       
	//RGB_ON(); //RGB exit standby
	mdelay(50);
	LCD_DATA(0x11);// LCD Exit Sleep
	mdelay(120);
	LCD_DATA(0x29);//lcd display on
#endif
...
//	LCD_DATA(0X00); 
	...
	LCD_COMM(0xC0);    //Power control 
	//LCD_DATA(0x21);
	LCD_DATA(0x1E);   //VRH[5:0] 
 
	LCD_COMM(0xC1);    //Power control //SAME AS DEFAULT
	LCD_DATA(0x11);   //SAP[2:0];BT[3:0] 
 
	LCD_COMM(0xC5);    //VCM control
	//LCD_DATA(0x4C); //4C	//5C
	//LCD_DATA(0x0C); //0C	//20  //1C
	LCD_DATA(0x31); 
	LCD_DATA(0x40); 
 
	LCD_COMM(0xC7);    //VCM control2 
	LCD_DATA(0XA6); 
 
	LCD_COMM(0x36);    // Memory Access Control 
	LCD_DATA(0x40);    //08=bgr 00==rgb

		
	LCD_COMM(0x3A);   
	LCD_DATA(0x55); 	// pixel format for 18-bit =60 16-bit=50
	...
	LCD_COMM(0xB6);    // Display Function Control 
	LCD_DATA(0x0A);   //0A 
	LCD_DATA(0x82);   //A2
        //LCD_DATA(0x27);   //no.of line of the lcd///320

	LCD_COMM(0xF6);  //interface control
	LCD_DATA(0x01); 
	LCD_DATA(0x00); 
	LCD_DATA(0x26);


	LCD_COMM(0xF2);    // 3Gamma Function Disable 
	LCD_DATA(0x00); 
 
	LCD_COMM(0x26);    //Gamma curve selected 
	LCD_DATA(0x01); 
 
	LCD_COMM(0xE0);    //Set Gamma 
	LCD_DATA(0x0F); 
	LCD_DATA(0x20); 
	LCD_DATA(0x1D); 
	LCD_DATA(0x0C); 
...
//      LCD_COMM(0x28);    //Display off
        mdelay(20);
//        LCD_COMM(0x10);   // entersleep 

        LCD_COMM(0x11);    //Exit Sleep 
        mdelay(120);

	LCD_COMM(0x29);    //Display on
	LCD_COMM(0x2c); 

	

printk("EXIT_STAND_BY LCD>>>>\n");
}
#endif
void LCD_Exit_Standby(void)
{
...
//	LCD_DATA(0X00); 
	...
	LCD_COMM(0x36);    // Memory Access Control 
	LCD_DATA(0x00);    //08=bgr 00==rgb

		
	LCD_COMM(0x3A);   
	LCD_DATA(0x60); 	// pixel format for 18-bit =60 16-bit=50
	...
	LCD_DATA(0x0A);   //0A 
//	LCD_DATA(0x82);
	LCD_DATA(0xA2);   //A2 rakesh
        LCD_DATA(0x27);   //no.of line of the lcd///320 rakesh
      //  LCD_DATA(0x82); //rakesh
		...
		struct fb_info *fb_info)
{
	...
	printk(">>>>>>>>>>>>>>>>>>>bits_per_pixel<<<<<<<<<<<<<< = %d", var->bits_per_pixel);
	switch (var->bits_per_pixel) {
	case 16:
		/* always expect RGB 565 */
		rgb = def_rgb565;
		printk("def_rgb565 was enabled");
		break;
	case 32:
...
/*################################################################################*/
/*	err = devm_gpio_request_one(&pdev->dev, 45,GPIOF_OUT_INIT_LOW, "SDA");
        if (err) {
                dev_err(&pdev->dev, "failed to get SDA-gpios: %d\n", err);
        }
        ret=gpio_get_value(45);
        printk("get_value = %d", ret);
        for(i=0;i<=10;i++){
                gpio_set_value(45, 1);
                mdelay(2);
                gpio_set_value(45, 0);
